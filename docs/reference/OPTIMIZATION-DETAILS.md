...
该子项独立管理以下单个 DWORD；查看当前值、关闭（写 `0`）或按首次快照恢复：

| 注册表路径 | 值名 | 类型 | 应用值 | 作用 |
|---|---|---|---|---|
| `HKLM:\SYSTEM\CurrentControlSet\Control\CI\Config` | VulnerableDriverBlocklistEnable | DWORD | 0 | 关闭内核级易受攻击驱动拦截 |
...
该计划源自作者本机正在使用的计划（名称 "kirby"，基于高性能模板深度调校），共 177 项设置。文件 `16384 bytes`，`SHA256 2EADB1A9A297C985A79100B1F1DBE994A2639D53C2D6A701CA019E5012868C7B`，完整来源与校验见 [电源计划来源与校验](../POWER-PLAN-SOURCE.md)。以下按子组列出全部内容（交流 = 接通电源，直流 = 电池供电）。
